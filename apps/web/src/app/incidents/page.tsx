    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={fetchIncidents} disabled={isLoading} className="gap-1.5 text-xs">
        ...
        <div className="flex items-center rounded-md border border-border bg-card p-0.5 text-xs">
          {["all", "triggered", "investigating", "acknowledged", "resolved"].map((st) => (
            <button
              ...
              className={`rounded px-3 py-1 uppercase font-mono font-medium transition-colors ${
                selectedStatus === st
        ...
        <div className="space-y-3">
          {incidents.map((inc) => (
            <Link key={inc.id} href={`/incidents/${inc.id}`} className="block">
              <Card className="hover:border-primary/50 transition-all p-5 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2 text-xs font-mono text-muted-foreground">
                      <Badge variant={getSeverityBadge(inc.severity)} className="uppercase text-[10px]">
                        {inc.severity}
                      </Badge>
                      <Badge variant={getStatusBadge(inc.status)} className="uppercase text-[10px]">
                        {inc.status}
                      </Badge>
                    ...
                    <h3 className="text-base font-bold text-foreground">{inc.title}</h3>
                    <p className="text-xs text-muted-foreground">{inc.triggerReason}</p>
                  </div>

                  <div className="flex items-center gap-3">
                    <Button size="sm" variant="default" className="gap-1.5 text-xs bg-primary hover:bg-primary/90">
